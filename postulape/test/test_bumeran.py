"""
Prueba aislada de Bumerán: scrapea y guarda en un Excel aparte,
con la MISMA estructura que el Excel general (usa excel_store).

Uso:
    python test_bumeran.py "asistente"
    python test_bumeran.py "asistente" 3      # 3 páginas
"""
...
from postulape.scrapers.bumeran import scrapear_bumeran
from postulape.storage import excel_store

# Excel de prueba, separado del general
RUTA_TEST = os.path.normpath(os.path.join(config.DIR_CVS, "..", "test_bumeran.xlsx"))
...
def main():
    ...
    print(f"Scrapeando Bumerán :: '{kw}' ({paginas} páginas)...\n")
    avisos = scrapear_bumeran(
        kw,
        config.UBICACION,
